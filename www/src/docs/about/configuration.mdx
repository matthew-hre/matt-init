...
* Scripts like `pnpm db:push`, `pnpm db:migrate`, and `pnpm dev` are set up for local development with Turso and Drizzle.
* `pnpm dev` runs both the dev server and a Turso dev DB (if enabled).
