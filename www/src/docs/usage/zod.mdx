...
> These docs are a bit outdated with the release of [@matthew-hre/env](https://www.npmjs.com/package/@matthew-hre/env), which abstracts away some of the complexity here. Check out the [package docs](https://www.npmjs.com/package/@matthew-hre/env) for the latest usage instructions.
...
- **Zod Schema**: We define a schema using `z.object()` that describes the expected environment variables, both server and client. This includes their types and any validation rules. In this case, we're only using server variables, and all variables are required strings.
- **Validation**: When the app starts, `loadEnv(schema)` checks that all required variables are present and valid.
