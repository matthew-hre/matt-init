...
Zod is a TypeScript-first schema validation library that ensures type safety at runtime. Every matt-init project uses Zod for environment variable validation and provides a foundation for form validation and API input validation.

## What's Included

### **Environment Variable Validation**
- **`src/lib/env.ts`** - Type-safe environment variable schema
- **Runtime validation** that fails fast on missing or invalid config
- **TypeScript types** automatically inferred from schema

### **Helper Utilities**
- **`src/lib/try-parse-env.ts`** - Environment parsing utilities
