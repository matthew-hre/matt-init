...
We've got one more thing to do before we can call it a day: set up authentication. Luckily, matt-init comes with a pre-configured authentication system using <Link>better-auth</Link>. Pop open a new terminal, (fire off a `nix develop` if needed), and run `pnpm db:push` to set up your database schema.
